...
- 목록 조회 시 `GET /posts?postType=${TABS[activeTab]}`로 전달한다.
...
- 비로그인 또는 게스트가 해당 액션 시 `AuthRequiredModal`을 띄운다.
  - `useAuthRequired` 훅의 `requireAuth(fn)` 패턴 사용.
...
- `PostResponse`에 `authorId`/`nickname` 필드가 없음 — 백엔드에서 제거됨.
- 댓글도 동일하게 익명 — `CommentItem`에 닉네임 없음.
- 자기 댓글 식별은 `comment.mine === true`로 처리한다 (삭제 버튼 표시 조건).

---

## 사진 첨부 표시 (CommunityPage)
...
`PostCreatePage`는 `PageLayout`을 사용하지 않는다. 직접 `div.h-dvh.flex.flex-col` 구조로 구성.
...
- 이미지: `ImageUploader domain="posts" aspectRatio={null}` — 비로그인 시 `requireAuth(() => {})` 트리거 버튼 표시
...
```jsx
<div className="w-full h-dvh flex flex-col overflow-hidden bg-white">
  <Header left={<BackIcon>} title="게시글 상세" right={<ShareIcon>} />
...
`PostComments.jsx`는 독립 컴포넌트로 댓글 목록 + 작성 입력창을 포함한다.
...
- 댓글 작성: `requireAuth(handleSubmit)` 래핑 — 비로그인 시 `AuthRequiredModal`
- Enter 키로 제출 (Shift+Enter는 줄바꿈, 제출 중에는 비활성)
- 삭제: `comment.mine === true`인 댓글에만 삭제 버튼(TrashIcon) 표시
- 삭제 성공 시 로컬 상태에서 즉시 제거 (낙관적 업데이트 없음 — API 성공 후 상태 갱신)
...
- 좋아요 버튼은 `requireAuth(handleLike)` 래핑 — 비로그인 시 AuthRequiredModal
- 좋아요 실패 시 에러 박스를 게시글 하단에 표시
...
```json
{
  ...
  "title": "오늘 점심",
  "content": "내용...",
  "imageUrl": "http://localhost:8080/images/posts/uuid.jpg"
}
```
...
| 상세 로딩 중 | "게시글을 불러오는 중..." |
| 상세 API 실패 | "게시글을 불러올 수 없어요" (h-40 중앙) |
| 상세 좋아요 실패 | 게시글 하단 에러 박스 |
| 댓글 로딩 중 | "댓글을 불러오는 중..." |
| 댓글 빈 결과 | "아직 댓글이 없어요" |
...
- 목록 페이지에 페이지네이션/무한스크롤이 들어가면 `useCommunityPosts.js`로 분리한다.
- 현재 페이지 3개 + 댓글 컴포넌트 구조 — 별도 상태 훅 추가는 페이지 로직이 100줄 초과할 때만.
